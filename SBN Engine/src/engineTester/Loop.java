...
public class Loop {
	...
	public Loop() throws IOException 
	{
		...
		ModelData sponzaData = OBJFileLoader.loadOBJ("/res/küp");
		...
		{
			float agacxx = random.nextFloat() * -15000;
			float agaczz = random.nextFloat() * -15000;
			float agacyy = terrain.getHeightOfTerrain(agacxx, agaczz);
			varliklar.add(new Entity(lowpolyagac,
			new Vector3f(agacxx,agacyy,agaczz), 0, random.nextFloat() * 360, 0, 3.0f));
		}
		
		for(int i=0;i<1000;i++) 
		{
			float cimenx = random.nextFloat() * -15000;
			float cimenz = random.nextFloat() * -15000;
			float cimeny = terrain.getHeightOfTerrain(cimenx, cimenz);
			varliklar.add(new Entity(cimen,
			new Vector3f(cimenx,cimeny,cimenz), 0, random.nextFloat() * 360, 0, 2.5f));
		}
		
		for(int i=0;i<1000;i++) 
		{
			float fernx = random.nextFloat() * -15000;
			float fernz = random.nextFloat() * -15000;
			float ferny = terrain.getHeightOfTerrain(fernx, fernz);
			varliklar.add(new Entity(fern,
			new Vector3f(fernx,ferny,fernz), 0, random.nextFloat() * 360, 0, 2.5f));
		}
		...
		while (!Display.isCloseRequested()) 
		{
				...
				if(menu) {
					if(editModu) 
					{
						...
						if(Mouse.isButtonDown(1)) {
							ÇizimVarliklar.add(new Entity(sponza, terrainPoint, 0, 0, 0, 10.0f));
			...
			renderer.processEntity(küp); // TODO: stencil test kaldýr
